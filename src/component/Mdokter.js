...
const MyComponent = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:5001/api/dokters', {
        headers: {
  ...
  const handleDelete = async (item) => {
    try {
      await axios.delete(`http://localhost:5001/api/dokters/${item._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        await axios.put(
          `http://localhost:5001/api/dokters/${editData.id}`,
          editData,
          config
        );
      } else {
        await axios.post('http://localhost:5001/api/dokters', newData, config);
      }
