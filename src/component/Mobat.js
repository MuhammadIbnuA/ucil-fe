...
const MyComponent = () => {
  ...
  const fetchObats = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/obats');
  ...
  const handleDelete = async (obat) => {
    try {
      const response = await fetch(`http://localhost:5001/api/obats/${obat._id}`, {
        method: 'DELETE',
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        const response = await fetch(`http://localhost:5001/api/obats/${editData.id}`, {
          method: 'PUT',
          headers: {
