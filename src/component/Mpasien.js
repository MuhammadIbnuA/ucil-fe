...
const PasienTable = () => {
  ...
  const fetchPasiens = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/pasiens', {
        headers: {
  ...
  const handleDelete = async (pasien) => {
    try {
      const response = await fetch(`http://localhost:5001/api/pasiens/${pasien._id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        const response = await fetch(`http://localhost:5001/api/pasiens/${editData.id}`, {
          method: 'PUT',
          headers: {
